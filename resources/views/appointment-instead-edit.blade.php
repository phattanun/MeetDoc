...
@section('title-inside')
    <a href="{{url('officer/appointment/edit')}}/{{$app->patient_id}}">แก้ไขการนัดหมายของ{{$patient->name}} {{$patient->surname}}</a> / แก้ไขการนัดหมาย รหัส {{$app->app_id}}
@endsection
    ...
    <div id="emailConfirmAlertModal" data-backdrop="static" class="modal fade" tabindex="-1" data-focus-on="input:first">
        ...
        <div class="modal-body">
            <div class="caption text-center">
                ...
                <span class="caption-subject font-red sbold uppercase">ระบบจะส่งจดหมายยืนยันการแก้ไขการนัดหมายไปทางอีเมล<br>และโทรศัพท์เคลื่อนที่ของผู้ป่วย กรุณาให้ผู้ป่วยยืนยันภายใน 1 วัน</span>
            </div>
        </div>
        <div class="modal-footer">
            <a href="{{url('officer/appointment/edit')}}/{{$app->patient_id}}" type="button" class="btn green">รับทราบ</a>
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#confirm-app-btn').click(function (e) {
                ...
                var l = Ladda.create(this);
                l.start();
                $.post('{{url('officer/appointment/edit')}}/{{$app->patient_id}}/submit',
